...
# The base Ruby image bundles its own copies of some gems (json and resolv
# as default gems, net-imap as a regular pre-installed gem) that can lag
...
# be removed explicitly rather than relying on gem uninstall. resolv is
# shipped as resolv-0.7.0.gemspec, which Trivy flags for CVE-2026-80212
# despite the Gemfile pinning resolv ~> 0.7.2 for `bundle exec` to load.
RUN find /usr/local/lib/ruby/gems -path "*/specifications/default/json-*.gemspec" -delete && \
  ...
  find /usr/local/lib/ruby/gems -path "*/specifications/default/resolv-*.gemspec" -delete && \
  find /usr/local/lib/ruby -maxdepth 2 -name "resolv.rb" -delete && \
  find /usr/local/lib/ruby -maxdepth 2 -type d -name "resolv" -exec rm -rf {} + && \
  find /usr/local/lib/ruby/gems -maxdepth 3 -type d -name "resolv-*" -exec rm -rf {} + && \
