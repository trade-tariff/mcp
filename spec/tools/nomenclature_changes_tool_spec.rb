...
RSpec.describe NomenclatureChangesTool do
  let(:base_url) { "https://example.com" }
  let(:body) { File.read("spec/fixtures/api/nomenclature_changes.json") }

  before { ENV["TARIFF_API_URL"] = base_url }
  after  { ENV.delete("TARIFF_API_URL") }

  it "calls the changes_by_period endpoint with date params" do
    stub = stub_request(:get, /uk\/api\/v2\/changes_by_period/)
  ...
  it "returns a response with changes and meta keys" do
    stub_request(:get, /uk\/api\/v2\/changes_by_period/)
      .to_return(status: 200, body: body, headers: { "Content-Type" => "application/json" })
