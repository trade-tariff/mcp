...
RSpec.describe ListCertificateTypesTool do
  let(:base_url) { "https://example.com" }
  let(:response_body) { File.read("spec/fixtures/api/certificate_types.json") }

  before { ENV["TARIFF_API_URL"] = base_url }
  after  { ENV.delete("TARIFF_API_URL") }

  it "calls the UK certificate types endpoint by default" do
    stub_request(:get, "#{base_url}/uk/api/v2/certificate_types")
  ...
  it "calls the XI endpoint when service is xi" do
    stub_request(:get, "#{base_url}/xi/api/v2/certificate_types")
      .to_return(status: 200, body: response_body, headers: { "Content-Type" => "application/json" })
  ...
  it "passes validity_date as as_of query param" do
    stub = stub_request(:get, "#{base_url}/uk/api/v2/certificate_types")
             .with(query: hash_including("as_of" => "2025-04-01"))
             .to_return(status: 200, body: response_body, headers: { "Content-Type" => "application/json" })

    described_class.call(validity_date: "2025-04-01")
