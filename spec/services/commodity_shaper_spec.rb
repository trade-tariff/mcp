...
RSpec.describe CommodityShaper do
  ...
  context "with a supplementary unit import measure" do
    let(:measure_type) { { "id" => "109", "type" => "measure_type", "attributes" => { "description" => "Supplementary unit" } } }
    let(:duty_expr)    { { "id" => "30000000-duty_expression", "type" => "duty_expression", "attributes" => { "base" => "p/st" } } }
    let(:geo_area)     { { "id" => "1011", "type" => "geographical_area", "attributes" => { "id" => "1011", "description" => "ERGA OMNES", "geographical_area_id" => "1011" } } }
    let(:measure) do
      {
        "id" => "30000000",
        ...
        "relationships" => {
          ...
          "duty_expression" => { "data" => { "id" => "30000000-duty_expression", "type" => "duty_expression" } },
          "geographical_area" => { "data" => { "id" => "1011", "type" => "geographical_area" } },
          "measure_conditions" => { "data" => [] }
        }
      }
    ...
    let(:response) do
      api_response(
        relationships: { "import_measures" => { "data" => [ { "id" => "30000000", "type" => "measure" } ] } },
        included: [ measure, measure_type, duty_expr, geo_area ]
      )
    end
    ...
    it "surfaces supplementary_unit at the top level of the commodity" do
      expect(shaped[:supplementary_unit]).to eq("p/st")
    end

    it "uses unit key instead of duty in the measure itself" do
      m = shaped[:import_measures].first
      expect(m[:unit]).to eq("p/st")
      expect(m).not_to have_key(:duty)
    end
  end

  context "with no supplementary unit measures" do
    it "omits supplementary_unit from the commodity" do
      expect(result).not_to have_key(:supplementary_unit)
    end
  end
